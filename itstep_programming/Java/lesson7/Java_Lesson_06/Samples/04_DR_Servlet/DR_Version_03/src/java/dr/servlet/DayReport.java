...
public class DayReport extends HttpServlet {
    ...
    private void doRequest(HttpServletResponse response,
        ...
        // создание потока для формирование HTTP-ответа клиенту
    ...
    /**
     * Генерация HTML-представления таблицы задач
     * выполненных пользователем за текущий день
     * @param writer - поток HTTP-ответа
     * @param todayTasks - список задач
     * @param totalHours - суммарное количество часов
     */
    private void renderTodayTasks(PrintWriter writer, List<Task> todayTasks, BigDecimal totalHours) {
        writer.println("<div>");
        writer.println("<h2>Reported Today</h2>");
        writer.println("<table>");
        writer.println("<thead>");
        writer.println("<tr>");
        writer.println("<th style='width:50px;'>Hours</th>");
        writer.println("<th style='width:150px;'>Reported At</th>");
        ...
        if (!todayTasks.isEmpty()) {
            ...
            for (Task task : todayTasks) {
                writer.println("<tr>");
                writer.println("<td>" + task.getHours() + "</td>");
                writer.println("<td>" + dateFormat.format(task.getReportedAt()) + "</td>");
                writer.println("<td>" + task.getDescription() + "</td>");
                writer.println("</tr>");
            }
        } else {
            writer.println("<tr><td colspan='3'>No data</td></tr>");
        }
        writer.println("</tbody>");
        if (totalHours != null) {
            ...
            writer.println("<b>Total Hours:&nbsp;</b>" + totalHours.toString());
            writer.println("</td>");
            writer.println("</tr>");
            writer.println("</tfoot>");
        }
