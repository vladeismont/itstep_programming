...
public class MySqlTaskRepository extends MySqlBaseRepository<Task, Integer> implements TaskRepository {
    ...
    public BigDecimal selectTotalHoursByDate(Date date) throws SQLException {
        Connection connection = createConnection();
        try {
            connection.setAutoCommit(false);
            Statement statement = connection.createStatement();
            try {
                ResultSet results = statement.executeQuery(selectTotalHoursByDateQuery(date));
                try {
                    results.next();
                    BigDecimal totalHours = results.getBigDecimal(1);
                    connection.commit();
                    return totalHours;
                } finally {
                    results.close();
    ...
    protected String selectTotalHoursByDateQuery(Date date) {
        DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        return "SELECT SUM(HOURS) FROM Task"
                + " WHERE DATE(ReportedAt)="
                + "'" + dateFormat.format(date) + "'";
    }
